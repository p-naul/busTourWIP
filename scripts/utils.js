...
function showBannerEN(isCorrect) {
  const banner = document.querySelector(".banner");
  const bannerText = document.querySelector(".banner-text");
  const messageSpan = banner.querySelector("span");
  ...
  if (isCorrect) {
    banner.style.backgroundColor = "#4ECB71"; // Green color for success
    messageSpan.textContent = "Well done! You selection is perfectly right";
  } else {
    banner.style.backgroundColor = "#ED7373"; // Red color for error
    messageSpan.textContent =
      "It was a good try! Unfortunately, your assembly is not stricly correct";
  }
